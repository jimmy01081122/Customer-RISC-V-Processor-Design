// Synthesizable floating-point helper for fsub, fmul, fcvt.w.s, fclass.
// Pure Verilog-2001 implementation (no real numbers). Rounding: nearest-even.
module fpu_unit (
    input  [1:0]  i_op,       // 0: fsub, 1: fmul, 2: fcvt.w.s, 3: fclass
    input  [31:0] i_a,
    input  [31:0] i_b,        // only used for fsub/fmul
    output reg [31:0] o_result,
    output reg       o_invalid,
    output reg       o_overflow,
    output reg       o_underflow
);
    localparam OP_FSUB   = 2'd0;
    localparam OP_FMUL   = 2'd1;
    localparam OP_FCVT   = 2'd2;
    localparam OP_FCLASS = 2'd3;

    // Classification helpers (pure bit checks)
    function is_nan;
        input [31:0] f;
    begin
        is_nan = (f[30:23] == 8'hff) && (f[22:0] != 0);
    end
    endfunction

    function is_inf;
        input [31:0] f;
    begin
        is_inf = (f[30:23] == 8'hff) && (f[22:0] == 0);
    end
    endfunction

    function is_zero;
        input [31:0] f;
    begin
        is_zero = (f[30:23] == 0) && (f[22:0] == 0);
    end
    endfunction

    function is_subnormal;
        input [31:0] f;
    begin
        is_subnormal = (f[30:23] == 0) && (f[22:0] != 0);
    end
    endfunction

    function is_negative;
        input [31:0] f;
    begin
        is_negative = f[31];
    end
    endfunction

    // fclass bitmask.
    function [31:0] do_fclass;
        input [31:0] f;
        reg [31:0] out;
    begin
        out = 32'b0;
        if (is_inf(f) && is_negative(f))                 out[0] = 1'b1;
        else if (!is_inf(f) && is_negative(f) && !is_subnormal(f) && !is_zero(f)) out[1] = 1'b1;
        else if (is_subnormal(f) && is_negative(f))      out[2] = 1'b1;
        else if (is_zero(f) && is_negative(f))           out[3] = 1'b1;
        else if (is_zero(f) && !is_negative(f))          out[4] = 1'b1;
        else if (is_subnormal(f) && !is_negative(f))     out[5] = 1'b1;
        else if (!is_inf(f) && !is_zero(f) && !is_subnormal(f) && !is_nan(f) && !is_negative(f)) out[6] = 1'b1;
        else if (is_inf(f) && !is_negative(f))           out[7] = 1'b1;
        else if (is_nan(f) && (f[22] == 1'b0))           out[8] = 1'b1; // signaling NaN
        else if (is_nan(f) && (f[22] == 1'b1))           out[9] = 1'b1; // quiet NaN
        do_fclass = out;
    end
    endfunction

    // Normalize and round mantissa with guard/sticky for add/sub/mul.
    task norm_round;
        input        sign_in;
        input [8:0]  exp_in;
        input [26:0] mant_in;  // [26:3] main 24 bits, [2] guard, [1] round, [0] sticky
        output reg [31:0] out_fp;
        output reg ovf;
        output reg udf;
    reg [8:0] exp_tmp;
    reg [24:0] mant_tmp;
    reg guard, roundb, sticky;
    reg incr;
    begin
        ovf = 1'b0;
        udf = 1'b0;
        exp_tmp  = exp_in;
        mant_tmp = {1'b0, mant_in[26:3]}; // 25 bits to detect carry
        guard    = mant_in[2];
        roundb   = mant_in[1];
        sticky   = mant_in[0];

        // Round to nearest even
        incr = guard & (roundb | sticky | mant_tmp[0]);
        if (incr) mant_tmp = mant_tmp + 1'b1;
        // Handle carry from rounding
        if (mant_tmp[24]) begin
            mant_tmp = {1'b0, mant_tmp[24:1]};
            exp_tmp  = exp_tmp + 1'b1;
        end

        // Handle overflow/underflow of exponent
        if (exp_tmp >= 9'd255) begin
            ovf     = 1'b1;
            out_fp  = {sign_in, 8'hff, 23'b0}; // INF
        end else if (exp_tmp <= 9'd0) begin
            udf     = 1'b1;
            out_fp  = 32'b0; // flush to zero
        end else begin
            out_fp = {sign_in, exp_tmp[7:0], mant_tmp[22:0]};
        end
    end
    endtask

    // Floating add/subtract (sub=1 -> a-b).
    task float_addsub;
        input  [31:0] a;
        input  [31:0] b;
        input         sub;
        output [31:0] res;
        output reg    ovf;
        output reg    udf;
    reg sign_a, sign_b, sign_res;
    reg [7:0] exp_a, exp_b;
    reg [23:0] mant_a, mant_b;
    reg [7:0] exp_large, exp_small;
    reg [24:0] mant_large;
    reg [26:0] mant_small_ext;
    reg [26:0] mant_small_shift;
    reg [8:0]  exp_diff;
    reg guard, roundb, sticky;
    reg [25:0] mant_sum;
    reg [25:0] mant_sub;
    reg [4:0]  lead_zero;
    reg [26:0] remainder;
    integer i;
    begin
        ovf = 1'b0; udf = 1'b0; res = 32'b0;
        sign_a = a[31]; sign_b = b[31] ^ sub;
        exp_a  = a[30:23]; exp_b = b[30:23];
        mant_a = (exp_a == 0) ? 24'b0 : {1'b1, a[22:0]};
        mant_b = (exp_b == 0) ? 24'b0 : {1'b1, b[22:0]};

        // Zero checks
        if ((exp_a == 0 && mant_a == 0) && (exp_b == 0 && mant_b == 0)) begin
            res = 32'b0;
            udf = 1'b1;
        end else if (exp_a == 0 && mant_a == 0) begin
            res = {sign_b, b[30:0]};
            udf = (exp_b == 0);
        end else if (exp_b == 0 && mant_b == 0) begin
            res = {sign_a, a[30:0]};
            udf = (exp_a == 0);
        end else begin
        // Identify larger exponent
        exp_large = 8'd0; exp_small = 8'd0;
        mant_large = 25'd0; mant_small_ext = 27'd0;
        guard = 1'b0; roundb = 1'b0; sticky = 1'b0;
        if (exp_a > exp_b || (exp_a == exp_b && mant_a >= mant_b)) begin
            exp_large = exp_a; mant_large = {mant_a,1'b0}; sign_res = sign_a;
            exp_small = exp_b; mant_small_ext = {mant_b,3'b0};
        end else begin
            exp_large = exp_b; mant_large = {mant_b,1'b0}; sign_res = sign_b;
                exp_small = exp_a; mant_small_ext = {mant_a,3'b0};
            end

        exp_diff = exp_large - exp_small;
        if (exp_diff == 0) begin
            mant_small_shift = mant_small_ext;
            guard  = 1'b0; roundb = 1'b0; sticky = 1'b0;
        end else if (exp_diff >= 27) begin
            mant_small_shift = 27'b0;
            guard = 1'b0; roundb = 1'b0; sticky = 1'b1;
        end else begin
            remainder = 27'd0;
            mant_small_shift = mant_small_ext >> exp_diff;
            guard  = mant_small_ext[exp_diff-1];
            remainder = mant_small_ext & ((27'd1 << exp_diff) - 1);
            roundb = 1'b0;
            sticky = |remainder;
        end

            if (sign_a == sign_b) begin
                mant_sum = {1'b0, mant_large} + {1'b0, mant_small_shift[26:2]};
                if (mant_sum[25]) begin
                    norm_round(sign_res, {1'b0,exp_large}+1'b1, {mant_sum[25:0], sticky}, res, ovf, udf);
                end else begin
                    norm_round(sign_res, {1'b0,exp_large}, {mant_sum[24:0], roundb, sticky}, res, ovf, udf);
                end
            end else begin
                mant_sub = {1'b0, mant_large} - {1'b0, mant_small_shift[26:2]};
                if (mant_sub == 0) begin
                    norm_round(1'b0, 9'd0, 27'b0, res, ovf, udf);
                end else begin : find_lead
                    lead_zero = 0;
                    for (i=25; i>=0; i=i-1)
                        if (mant_sub[i] && lead_zero == 0)
                            lead_zero = 25 - i;
                    mant_sub = mant_sub << lead_zero;
                    norm_round(sign_res, {1'b0,exp_large} - lead_zero[8:0], {mant_sub[25:0], sticky}, res, ovf, udf);
                end
            end
        end
    end
    endtask

    // Floating multiply
    task float_mul;
        input  [31:0] a;
        input  [31:0] b;
        output [31:0] res;
        output reg    ovf;
        output reg    udf;
    reg sign;
    reg [8:0] exp_sum;
    reg [23:0] mant_a, mant_b;
    reg [47:0] mant_prod;
    reg [23:0] mant_norm;
    reg [8:0] exp_norm;
    reg guard, roundb, sticky;
    begin
        ovf = 1'b0; udf = 1'b0; res = 32'b0;
        if (is_zero(a) || is_zero(b)) begin
            res = 32'b0;
        end else begin
        sign    = a[31] ^ b[31];
        mant_a  = (a[30:23]==0) ? 24'b0 : {1'b1, a[22:0]};
        mant_b  = (b[30:23]==0) ? 24'b0 : {1'b1, b[22:0]};
        exp_sum = a[30:23] + b[30:23] - 9'd127;

        mant_prod = mant_a * mant_b; // 48 bits
        // Normalize: product is in [1,4)
        if (mant_prod[47]) begin
            mant_norm = mant_prod[47:24];
            guard     = mant_prod[23];
            roundb    = mant_prod[22];
            sticky    = |mant_prod[21:0];
            exp_norm  = exp_sum + 1'b1;
        end else begin
            mant_norm = mant_prod[46:23];
            guard     = mant_prod[22];
            roundb    = mant_prod[21];
            sticky    = |mant_prod[20:0];
            exp_norm  = exp_sum;
        end

            norm_round(sign, exp_norm, {mant_norm, guard, roundb, sticky}, res, ovf, udf);
        end
    end
    endtask

    // Float to signed int conversion (round to nearest even).
    task fcvt_w_s;
        input  [31:0] a;
        output [31:0] out_int;
        output reg    inv;
    reg sign;
    reg [7:0] exp;
    reg [23:0] mant;
    integer exp_unbias;
    reg [55:0] shifted;
    reg guard, sticky;
    reg [31:0] int_mag;
    begin
        inv  = 1'b0;
        out_int = 32'b0;
        sign = a[31];
        exp  = a[30:23];
        mant = (exp==0) ? 24'b0 : {1'b1, a[22:0]};
        if (is_nan(a) || is_inf(a)) begin
            inv = 1'b1;
            out_int = 32'b0;
        end
        exp_unbias = exp - 127;
        if (!inv) begin
            if (exp_unbias > 30) begin
                inv     = 1'b1;
                out_int = sign ? 32'h8000_0000 : 32'h7fff_ffff;
            end else if (exp_unbias < 0) begin
                out_int = 32'b0;
            end else begin
                if (exp_unbias >= 23) begin
                    int_mag = mant << (exp_unbias - 23);
                    guard   = 1'b0;
                    sticky  = 1'b0;
                end else begin
                    shifted = {mant,32'b0};
                    shifted = shifted >> (23 - exp_unbias);
                    int_mag = shifted[55:24];
                    guard   = shifted[23];
                    sticky  = |shifted[22:0];
                end
                // Round to nearest even
                if (guard && (sticky || int_mag[0])) int_mag = int_mag + 1'b1;
                out_int = sign ? (~int_mag + 1'b1) : int_mag;
            end
        end
    end
    endtask

    // ---------------------------
    // Main combinational logic
    // ---------------------------
    always @(*) begin
        o_result    = 32'b0;
        o_invalid   = 1'b0;
        o_overflow  = 1'b0;
        o_underflow = 1'b0;

        if (i_op == OP_FCLASS) begin
            o_result  = do_fclass(i_a);
        end else if (i_op == OP_FCVT) begin
            fcvt_w_s(i_a, o_result, o_invalid);
        end else if (i_op == OP_FMUL) begin
            if (is_nan(i_a) || is_nan(i_b) || is_inf(i_a) || is_inf(i_b)) begin
                o_invalid = 1'b1;
                o_result  = 32'b0;
            end else begin
                float_mul(i_a, i_b, o_result, o_overflow, o_underflow);
                o_invalid = o_overflow | o_underflow;
            end
        end else begin
            // FSUB
            if (is_nan(i_a) || is_nan(i_b) || is_inf(i_a) || is_inf(i_b)) begin
                o_invalid = 1'b1;
                o_result  = 32'b0;
            end else begin
                float_addsub(i_a, i_b, 1'b1, o_result, o_overflow, o_underflow);
                o_invalid = o_overflow | o_underflow;
            end
        end
    end
endmodule
